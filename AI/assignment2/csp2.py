...
def main():
    if len(sys.argv) < 3:
        print("Usage: python sudoku.py <puzzle (1-4)> <method (1:BT, 2:BT+FC, 3:BT+FC+Heuristics)> [<num_trials>]")
        return

    puzzle_index = int(sys.argv[1]) - 1
    method_type = int(sys.argv[2])
    n_trials = int(sys.argv[3]) if len(sys.argv) == 4 else 1
    ...
    for i in range(n_trials):
        ...
        if method_type == 1:
            print("Using backtracking search.")
            result = solve_backtrack(puzzle_copy)
        elif method_type == 2:
            print("Using backtracking with forward checking.")
            result = solve_btfc(puzzle_copy)
        else:
            print("Using backtracking with forward checking and heuristics.")
            result = solve_btfch(puzzle_copy)
        ...
        if not result:
            ...
            print(f"Trial {i+1}: Solved in {elapsed_time:.4f} seconds")

    if n_trials > 1:
        avg_time = sum(run_times) / n_trials
        print(f"\nAverage time over {n_trials} runs: {avg_time:.4f} seconds")
...
def print_help():
    print("Usage: python sudoku.py <difficulty=1,2,3,4> <method=1,2,3> [optional: no. of trials]")        
