...
def user_input():
    ...
    percent_income = st.number_input("Income to Debt ratio", value=percent_income_ratio(income, amount), disabled=True)
...
# # With color (using HTML):
#     if prediction[0] == 1:
#         st.markdown('<span style="color:red">*High Credit Risk*</span>', unsafe_allow_html=True)
#     else:
#         st.markdown('<span style="color:green">*Low Credit Risk*</span>', unsafe_allow_html=True)
#         #st.markdown("**Credit Risk:**", "High" if prediction[0] == 1 else "Low", unsafe_allow_html=False, *, help=None)
